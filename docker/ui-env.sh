...
#                       and window.__APP_BASE_PATH__ (packages/ui/src/lib/appBase.ts).
...
# (packages/ui/tests/ui-env-script.test.ts) points them at a temporary copy.
set -eu

html="${UI_ENV_HTML:-/usr/share/nginx/html}"
conf="${UI_ENV_NGINX_CONF:-/etc/nginx/nginx.conf}"
...
for name in AUTH_URL JOBS_URL REPORT_URL; do
  ...
  printf '%s' "$value" | grep -Eq '^https?://[A-Za-z0-9.-]+(:[0-9]+)?(/[A-Za-z0-9._~/-]*)?$' \
    || { echo "ui-env: $name must be a plain http(s) URL, got: $value" >&2; exit 1; }
...
grep -qF "connect-src 'self' $origins;" "$conf" || { echo "ui-env: nginx.conf carries no __SERVICE_ORIGINS__ to fill" >&2; exit 1; }
