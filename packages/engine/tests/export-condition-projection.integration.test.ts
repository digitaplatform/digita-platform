...
// Viewer's read grant excludes cancelled (docstatus 2) rows via a condition.
const COND: EntityDefinition = {
  name: "CondDoc",
  module: "test",
  database: "app",
  ...
  permissions: [
    { role: "Administrator", level: 0, select: 1, read: 1, write: 1, create: 1, delete: 1, export: 1 },
    { role: "Viewer", level: 0, select: 1, read: 1, export: 1, condition: "eval:doc.docstatus != 2" },
  ],
...
beforeAll(async () => {
  ...
  // Insert two docs directly: A open (docstatus 1), B cancelled (docstatus 2).
  await db.insertOne("CondDoc", { _id: "A", title: "open", docstatus: 1, owner: "v@d" }, "app");
  await db.insertOne("CondDoc", { _id: "B", title: "cancelled", docstatus: 2, owner: "v@d" }, "app");

  viewerTok = await ta.sign({ sub: "v@d", email: "v@d", roles: ["Viewer", "System User"], tiers: ["internal"] });
}, 90000);
